...
import com.example.plantasks.Adapter.ToDoAdapter;
...
public class RecyclerItemTouchHelper extends ItemTouchHelper.SimpleCallback {
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if (dX > 0) {
            icon = ContextCompat.getDrawable(adapter.getContext(), R.drawable.baseline_add_task);
            background = new ColorDrawable(ContextCompat.getColor(adapter.getContext(), R.color.black));
        } else {
            icon = ContextCompat.getDrawable(adapter.getContext(), R.drawable.baseline_10mp_24);
